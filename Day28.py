...
if __name__ == '__main__':
    ...
    strategy = lambda:0
    strategy.name='BBAND'
    strategy.obj=strategy_BBAND()
    strategy.range=list_BBAND_range
    ...
    strategy = lambda:0
    strategy.name='MA'
    strategy.obj=strategy_MA()
    strategy.range=list_MA_range
    strategylist.append(strategy)
    
    optimizeResult=optimizeListOfStrategies(kbars,strategylist)
    
    mdd_BBAND=calculatMDD(prefixProd(optimizeResult['BBAND'][3]))
    mdd_MA=calculatMDD(prefixProd(optimizeResult['MA'][3]))
    print('BBAND MDD:',mdd_BBAND)
    print('MA MDD:',mdd_MA)