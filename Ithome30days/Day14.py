...
class strategy_SAR:
    ...
    def __init_(self):
        pass
    def createsignal(kbars_daily,args):
...
class strategy_MA:
    ...
    def bodyfor_optimize(self,args,previouslist):
        ...
        if retStrategy>bestret:
            ...
            args['bestargs']={'periodLong':periodLong,'periodShort':periodShort}
...
def strategy_optimize(kbars_daily,strategy_obj,rangelist):
    bestret=0
    bestargs=strategy_obj.argsDefault
    bestbuy=[]
    bestretseries=[]
    ...
    bestargs=args_pack['bestargs']
    bestbuy=args_pack['bestbuy']
    bestretseries=args_pack['bestretseries']
    return bestret,bestargs,bestbuy,bestretseries
...
#oil
BRF=api.Contracts.Futures.BRF
print(BRF._name)

#find name
d = dir(BRF)
df=pd.read_csv('SYMBOL.csv')  
print(df.loc[df['symbol'] == BRF._name])
...
def createFutureNameLookup():
    ...
    for i in range(0,len(l),1):
        if(len(l[i])>3):
            ...
            name=df.loc[df['symbol'] == l[i]]['name'].item()
            reference[l[i]]=name
    return reference
LookUp=createFutureNameLookup()
LookUp['TXF']
...
dueDate='202110'
contract=0
exec('contract = api.Contracts.Futures.'+contractName+'.'+contractName+dueDate)
print(contract)
kbars=getContractKbars(contract,start="2010-01-01",end="2021-09-14")
kbars0050=getStockKbars('0050',start="2010-01-01",end="2021-09-14")
kbars006204=getStockKbars('006204',start="2010-01-01",end="2021-09-14")
...
ret=strategy_optimize(kbars_daily,MA,list_MA_range)
ret2=optimizeMA(dayopen0050,dayclose0050,rangeLong=range(20,60,2),rangeShort=range(20,60,2))
