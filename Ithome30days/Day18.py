...
closemax=TW_close.max()
closemin=TW_close.min()
buy=1.0-(TW_close-closemin)/(closemax-closemin)
profitTW,retseriesTW=backtest_signal(TW_open,buy)
prefix=prefixProd(retseriesTW)
plt.plot(prefix,color='green')#近期走勢

#確認相容性
buyMA= maSignal(TW_close)
profitMA,retseriesMA=backtest_signal(TW_open,buyMA)
