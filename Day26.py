...
if __name__ == '__main__':
    ...
    MACD=strategy_MACD()
    retMACD=strategy_optimize(kbars,MACD,list_MACD_range)
    
    
    fastperiod=retMACD[1]['fastperiod']
    slowperiod=retMACD[1]['slowperiod']
    signalperiod=retMACD[1]['signalperiod']
    list_MACD_range=\
        [np.arange(fastperiod,fastperiod+1,1)\
         ,np.arange(slowperiod,slowperiod+1,1)\
         ,np.arange(5,20,1)
         ]
    retMACD=strategy_optimize(kbars,MACD,list_MACD_range)
    
    bestret=retMACD[0]
    bestparameter=retMACD[1]
    bestbuy=retMACD[2]
    bestretseries=retMACD[3]
    print(bestret)
    prefixProfit=prefixProd(bestretseries)
    plt.plot(prefixProfit)